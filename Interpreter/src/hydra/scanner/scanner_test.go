...
var mocks = []mock_input{
	{
       ...
       print(i)
		...
		[]token.Token_Type{
			token.NUM_LITERAL, token.EXCL_RANGE, token.NUM_LITERAL,
	...
	},
	{
       ...
       this.c = a ? (a | b) : (b & a);
		...
		[]token.Token_Type{
			...
			token.THIS_KEYWORD, token.PERIOD, token.IDENTIFIER, //token.SUPER_KEYWORD,
	...
	{
		`return (a += 2, b *= 2, c -= 2, d /= 2, e %= 2, f ^= 2, g &= 1, h |= 1)`,
		[]token.Token_Type{
			...
			token.COMMA, token.IDENTIFIER, token.POWER_EQ,
			token.NUM_LITERAL, token.COMMA, token.IDENTIFIER,
