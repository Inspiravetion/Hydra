...
class MarkdownCompiler():
    ...
    def get_highlight(self):
        ...
        if self.settings.get('enable_highlight') is True and self.settings.get('parser') == 'default':
            custom_css = self.settings.get('custom_css_file')
            if custom_css:
                highlight += "<style>%s</style>" % load_resource(custom_css)    
            else:   
                highlight += "<style>%s</style>" % load_resource('highlight.css')
            highlight += "<script>%s</script>" % load_resource('highlight.js')
            
            custom_syntax = self.settings.get('custom_highlight_file')
            if custom_syntax:
                highlight += "<script>%s</script>" % load_resource(custom_syntax)
